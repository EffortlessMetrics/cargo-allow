...
| policy dialect [#1470](https://github.com/EffortlessMetrics/cargo-allow/issues/1470) | Discovery prefers `policy/cargo-allow.toml`, recognizes the `policy = "cargo-allow"` dialect marker, and skips foreign-dialect `policy/allow.toml` with named diagnostics; ub-review dogfood receipts and import-mode parity (#1466) still open for full adoption | partial | repo-infra | #1699 merge `53ea19aa`; `policy_discovery` integration tests; `allow-policy` discovery unit tests | B6 |
