...
pub fn write_file(path: impl AsRef<Path>, contents: &str) -> RepoEditResult<()> {
    let path = path.as_ref();
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).map_err(|e| {
            RepoEditError::new(format!("failed to create {}: {e}", parent.display()))
        })?;
    }
...
pub fn write_file_no_overwrite(
    ...
    file.flush().map_err(|error| {
        let _ = fs::remove_file(path);
        RepoEditError::new(format!("failed to flush {}: {error}", path.display()))
    })?;
    file.sync_all().map_err(|error| {
        let _ = fs::remove_file(path);
        RepoEditError::new(format!("failed to sync {}: {error}", path.display()))
    })?;
    sync_parent_directory(path)
}
...
pub fn write_file_create_new_atomic(path: impl AsRef<Path>, contents: &str) -> RepoEditResult<()> {
...
pub fn write_file_create_new_atomic_with_permissions(
    ...
    let (tmp, mut file) = create_unique_temp(path)?;
    let prepare_result: RepoEditResult<()> = (|| {
        file.write_all(contents.as_bytes()).map_err(|error| {
            RepoEditError::new(format!("failed to write {}: {error}", tmp.display()))
        })?;
        file.flush().map_err(|error| {
            RepoEditError::new(format!("failed to flush {}: {error}", tmp.display()))
        })?;
        ...
        file.sync_all().map_err(|error| {
            RepoEditError::new(format!("failed to sync {}: {error}", tmp.display()))
        })?;
        Ok(())
    })();
...
fn create_unique_temp(path: &Path) -> RepoEditResult<(PathBuf, std::fs::File)> {
    let base_name = path
        .file_name()
...
#[cfg(not(unix))]
fn sync_parent_directory(_path: &Path) -> RepoEditResult<()> {
    Ok(())
}
