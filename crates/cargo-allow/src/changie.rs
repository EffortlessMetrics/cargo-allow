...
pub(crate) enum ChangieCommand {
    ...
    /// Emit the repository's config-derived fragment JSON Schema (#3613).
...
pub(crate) struct ChangieSchemaArgs {
    ...
    /// (YAML Language Server, VS Code YAML, coc-yaml) instead of the
    /// schema itself. Config documents stay associated with Changie's
    ...
    /// Write output to a caller-selected file. The source tree is
    /// never mutated.
    #[arg(long)]
...
pub(crate) fn cmd_changie(args: &ChangieArgs) -> CargoAllowResult<()> {
    match &args.command {
        ChangieCommand::Lint(lint) => cmd_changie_lint(args, lint),
        ChangieCommand::Schema(schema) => cmd_changie_schema(args, schema),
    }
}

fn cmd_changie_schema(args: &ChangieArgs, schema: &ChangieSchemaArgs) -> CargoAllowResult<()> {
    let cwd = current_dir()?;
