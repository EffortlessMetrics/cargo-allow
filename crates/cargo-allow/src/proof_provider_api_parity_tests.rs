...
#[test]
fn proof_provider_api_parity_fixtures_registered() -> Result<(), String> {
    let root = repo_root();
    for path in proof_provider_api_parity_fixture_paths(&root) {
        if !path.is_file() {
    ...
    if !ledger.contains("introduce-proof-provider-api-crate") {
        return Err("move ledger missing proof-provider-api scaffold entry".to_string());
    }

    let manifest = root.join("crates/cargo-allow/Cargo.toml");
    let manifest_text = std::fs::read_to_string(&manifest)
        .map_err(|err| format!("read cargo-allow manifest: {err}"))?;
    if manifest_lists_dependency(&manifest_text, "proof-provider-api") {
...
fn proof_provider_api_parity_fixture_paths(root: &std::path::Path) -> Vec<PathBuf> {
    vec![root.join("tests/fixtures/proof-provider-api/parity-boundary-v1.toml")]
}
