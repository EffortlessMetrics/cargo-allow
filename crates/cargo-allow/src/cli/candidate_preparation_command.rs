...
pub(crate) fn apply_candidate_plan(
    ...
    let mut staged: Vec<(&allow_report::CandidateFileOperationV1, Vec<u8>)> = Vec::new();
    for operation in &write_ops {
        let Some(surface) = rendered
            .iter()
            .find(|surface| surface.owner == operation.owner && surface.path == operation.path)
        else {
            receipt.state = CandidateApplyStateV1::Mismatch;
            receipt.reasons.push(format!(
                "operation {}:{} vanished from the re-rendered surface set",
                operation.owner, operation.path
            ));
            return receipt;
        };
        // Only deterministic surfaces reach this loop, so the rendered
        // bytes are always present and UTF-8 by construction.
        ...
        let contents = match std::str::from_utf8(bytes) {
            ...
            Err(error) => {
                abort = Some((
                    ...
                    format!("{} staged bytes are not UTF-8: {error}", operation.path),
                ));
                break;
            }
        };
    ...
    for (operation, bytes) in &staged {
        let after_digest = allow_core::sha256_v1_bytes(bytes);
