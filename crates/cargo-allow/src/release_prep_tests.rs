...
const RELEASE_DOC: &str = "docs/release/0.1.3.md";
...
#[test]
fn release_0_1_3_version_handoff_matches_workspace_versions() {
    let root = workspace_root();
    ...
    assert!(
        release_doc.contains("# 0.1.3 Release Record"),
        "release record should name the completed patch release"
    );
    assert!(
        release_doc.contains("from `0.1.2` to `0.1.3`"),
        "release record should document the completed version bump"
    );
    assert!(
        release_doc.contains("--version 0.1.3"),
