...
#[test]
fn proof_protocol_parity_fixtures_registered() -> Result<(), String> {
    let root = repo_root();
    for path in proof_protocol_parity_fixture_paths(&root) {
        if !path.is_file() {
            return Err(format!("missing parity fixture {}", path.display()));
        }
    }

    let doc = root.join("docs/architecture/proof-protocol.md");
    let doc_text =
        std::fs::read_to_string(&doc).map_err(|err| format!("proof-protocol doc: {err}"))?;
    ...
    if manifest_lists_dependency(&manifest_text, "proof-protocol") {
