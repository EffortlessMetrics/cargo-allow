use std::path::PathBuf;

#[test]
fn intent_edit_parity_fixtures_registered() -> Result<(), String> {
    let root = repo_root();
    for path in intent_edit_parity_fixture_paths(&root) {
        if !path.is_file() {
            return Err(format!("missing parity fixture {}", path.display()));
        }
    }

    let doc = root.join("docs/architecture/intent-edit.md");
    let doc_text =
        std::fs::read_to_string(&doc).map_err(|err| format!("intent-edit doc: {err}"))?;
    if !doc_text.contains("2613-A") {
        return Err("human projection missing packet marker".to_string());
    }
    if !doc_text.contains("2613-B") {
        return Err("human projection missing edit-plan packet marker".to_string());
    }
    if !doc_text.contains("2613-C") {
        return Err("human projection missing dialect/approval packet marker".to_string());
    }
    if !doc_text.contains("2613-D") {
        return Err("human projection missing repo-edit translation marker".to_string());
    }
    if !doc_text.contains("2613-E") {
        return Err("human projection missing recompile contract marker".to_string());
    }
    if !doc_text.contains("2613-F") {
        return Err("human projection missing settlement marker".to_string());
    }

    let ledger = std::fs::read_to_string(root.join("policy/product-move-ledger.toml"))
        .map_err(|err| format!("move ledger: {err}"))?;
    if !ledger.contains("introduce-intent-edit-crate") {
        return Err("move ledger missing intent-edit scaffold entry".to_string());
    }
    if !ledger.contains("introduce-intent-edit-plan") {
        return Err("move ledger missing intent-edit plan entry".to_string());
    }
    if !ledger.contains("introduce-intent-edit-dialect-approval") {
        return Err("move ledger missing intent-edit dialect/approval entry".to_string());
    }
    if !ledger.contains("introduce-intent-edit-repo-edit-translation") {
        return Err("move ledger missing intent-edit repo-edit translation entry".to_string());
    }
    if !ledger.contains("introduce-intent-edit-recompile-contract") {
        return Err("move ledger missing intent-edit recompile contract entry".to_string());
    }
    if !ledger.contains("introduce-intent-edit-settlement") {
        return Err("move ledger missing intent-edit settlement entry".to_string());
    }

    let manifest = root.join("crates/cargo-allow/Cargo.toml");
    let manifest_text = std::fs::read_to_string(&manifest)
        .map_err(|err| format!("read cargo-allow manifest: {err}"))?;
    if manifest_lists_dependency(&manifest_text, "intent-edit") {
        return Err("cargo-allow must not depend on intent-edit".to_string());
    }

    Ok(())
}

fn intent_edit_parity_fixture_paths(root: &std::path::Path) -> Vec<PathBuf> {
    vec![
        root.join("tests/fixtures/intent-edit/parity-boundary-v1.toml"),
        root.join("tests/fixtures/intent-edit/parity-edit-plan-v1.toml"),
        root.join("tests/fixtures/intent-edit/parity-dialect-adapter-v1.toml"),
        root.join("tests/fixtures/intent-edit/parity-approval-currentness-v1.toml"),
        root.join("tests/fixtures/intent-edit/parity-repo-edit-translation-v1.toml"),
        root.join("tests/fixtures/intent-edit/parity-recompile-contract-v1.toml"),
        root.join("tests/fixtures/intent-edit/parity-settlement-v1.toml"),
    ]
}

fn manifest_lists_dependency(manifest_text: &str, crate_name: &str) -> bool {
    for section in ["dependencies", "dev-dependencies"] {
        let Ok(table) = toml::from_str::<toml::Table>(manifest_text) else {
            continue;
        };
        let Some(deps) = table.get(section).and_then(|value| value.as_table()) else {
            continue;
        };
        if deps.contains_key(crate_name) {
            return true;
        }
    }
    false
}

fn repo_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..")
}
