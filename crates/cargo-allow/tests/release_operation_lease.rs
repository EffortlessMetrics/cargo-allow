...
use allow_report::{
    ...
    OperationLeaseClassV1, OperationLeaseKeyV1, OperationLeaseStateV1, acquire_operation_lease_v1,
    ...
    release_operation_lease_v1, render_release_operation_lease_v1, renew_operation_lease_v1,
    verify_lease_readback_v1,
};
...
fn release_operation_lease_concurrency() -> Result<(), Box<dyn Error>> {
    ...
    // Control: clean and recovery runs never overlap on one subject.
    ...
    require(
        ...
        "recovery derives a distinct operation key",
    )?;
    ...
    let next = acquire_operation_lease_v1(acquire_init(key()), Some(&record), EXPIRES_AT + 100)
        .map_err(io::Error::other)?;
    require(
...
fn release_operation_lease_runner_loss() -> Result<(), Box<dyn Error>> {
    ...
    observe_runner_loss_v1(&mut record, NOW + 20).map_err(io::Error::other)?;
    require(
    ...
    let mut record = acquired()?;
    observe_runner_loss_v1(&mut record, NOW + 10).map_err(io::Error::other)?;
    require(
        record.state == State::ExpiredPreIrreversible,
