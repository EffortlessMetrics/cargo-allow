...
fn require_contains(text: &str, needle: &str, label: &str) -> Result<(), String> {
    if text.contains(needle) {
        ...
        Err(format!(
            "{label} is missing required generation-2 text: {needle}"
        ))
    }
}
...
fn three_product_generation_two_reconstructs_current_authority() -> Result<(), String> {
    ...
    let expected_product_ids = ["cargo-allow", "cargo-intent", "cargo-proof"];
    if fixture.product.len() != expected_product_ids.len() {
        return Err(format!(
            "expected {} product rows, got {}",
            expected_product_ids.len(),
            fixture.product.len()
        ));
    }
    let mut seen_products = BTreeSet::new();
    for row in &fixture.product {
        if !seen_products.insert(row.id.as_str()) {
            return Err(format!("duplicate product ID {}", row.id));
        }
        if !expected_product_ids.contains(&row.id.as_str()) {
            return Err(format!("unexpected product ID {}", row.id));
        }
    }
    ...
    for (artifact, disposition) in required_dispositions {
        let entry = fixture
            ...
            .ok_or_else(|| format!("fixture is missing disposition {artifact} = {disposition}"))?;
