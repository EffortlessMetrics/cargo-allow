use super::{
    CompiledSpecGraph, EvidenceSubjectId, ImplementationSliceClass, ImplementationSliceId,
    ImplementationSliceV1, RequirementStatus, SupportClaimDispositionState,
};
...
/// Evaluate objective, source-tree facts for one exact staged candidate.
///
/// This function only consumes already-normalized facts. It does not read Git,
/// inspect the worktree, execute tests, or mutate requirement/support state.
...
fn validate_slice(
    ...
    for seam in slice.owned_seams.iter().chain(slice.shared_seams.iter()) {
        if !candidate
            .seams
            .contains_key(&super::ImplementationSeamId(seam.clone()))
        {
            push_finding(
                findings,
    ...
    for claim in claims {
        ...
        for subject_id in &claim.subject_ids {
            ...
            match candidate.subjects.get(subject_id) {
                ...
                Some(subject) if subject.role != super::EvidenceSubjectRole::ExactEvidence => {
                    push_finding(
                        findings,
                        PrecommitFindingCode::ExactSelectorMissing,
                        subject_id.as_str(),
    ...
    let evidence_closed = slice.evidence.state == super::EvidenceDispositionState::Current
        && slice
            .evidence
            .receipt
            .as_deref()
...
mod tests {
    use super::*;
    use crate::spec_system::{
        GraphCompileInput, ImplementationSeamId, ImplementationSeamRegistration, SourceLocation,
        compile_spec_graph, parse_implementation_slice, parse_requirement_blocks,
    };
...
"#;
    ...
    fn spec_only_cannot_publish_implemented_runtime_claim() -> Result<(), String> {
        ...
        slice.implementation_claim.status = super::super::ImplementationClaimStatus::Implemented;
        let declaration = PrecommitChangeDeclaration {
