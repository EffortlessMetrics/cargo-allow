...
pub(super) fn collect_line_scopes(
    ...
    collect_node_scopes(node, source, &mut module_path, &mut impl_path, scopes);
}

fn collect_node_scopes(
    node: Node<'_>,
    source: &str,
...
fn visit_child_scopes(
    ...
    for child in node.children(&mut cursor) {
        collect_node_scopes(child, source, module_path, impl_path, scopes);
    }
}
...
fn record_module_scope(
    ...
    record_scope_span(node, module_path, None, scopes);
}
...
fn record_container_scope(
    ...
    record_scope_span(node, module_path, Some(name), scopes);
}

fn record_scope_span(
    node: Node<'_>,
    module_path: &[String],
