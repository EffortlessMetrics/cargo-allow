...
fn manifest_concat_uses_the_structural_argument_and_all_delimiters() -> Result<(), String> {
    let scan = scan_rust_source_coupling(
        "r#include_str!(/* concat!(env!(\"CARGO_MANIFEST_DIR\"), \"/decoy.rs\") */ concat /* macro */ ! {env /* base */ ! [\"CARGO_MANIFEST_DIR\"] /* base tail */, /* outer, ) [ /* nested */ */ \"/actual.rs\" /* literal tail */});\ninclude_str!(concat![env!{\"CARGO_MANIFEST_DIR\"}, // comma, close )\n \"/bracket.rs\"]);\n",
    )
