//! Spec-system report rendering (#3522 slice C).
//!
//! The spec-system view/render projections moved out of cargo-allow's
//! embedded authority. Rendering is pure over the
//! [`crate::spec_system_report_types`] DTOs; output formats are
//! unchanged (human/markdown/json/sarif/html). Failure posture and
//! blocking counts come from the DTO methods so every consumer computes
//! the same answer.

use allow_core::{CargoAllowError, CargoAllowErrorKind};

#[cfg(test)]
use crate::spec_system_report_types::SpecSystemFinding;
use crate::spec_system_report_types::{
    SpecSystemArtifact, SpecSystemLink, SpecSystemReport, SpecSystemWorkItem,
...
/// Neutral output-format selector for spec-system rendering. cargo-allow
/// maps its CLI `OutputFormat` onto this; other products can render the
/// same DTOs without cargo-allow's CLI types.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SpecSystemRenderFormat {
    Human,
    Html,
    Json,
    Sarif,
...
pub fn render_spec_system_report(
    report: &SpecSystemReport,
    format: SpecSystemRenderFormat,
) -> String {
    match format {
        SpecSystemRenderFormat::Json => render_spec_system_json(report),
        SpecSystemRenderFormat::Html => format!(
        ...
        SpecSystemRenderFormat::Sarif => render_spec_system_sarif(report),
        SpecSystemRenderFormat::Human | SpecSystemRenderFormat::Markdown => {
            render_spec_system_markdown(report)
        }
    }
}

pub fn filter_spec_system_report_for_artifact(
    report: &SpecSystemReport,
    artifact_id: &str,
) -> Result<SpecSystemReport, CargoAllowError> {
    let artifact = report
        .artifacts
        .iter()
...
pub fn render_spec_system_explain_markdown(report: &SpecSystemReport) -> String {
...
pub fn render_spec_system_markdown(report: &SpecSystemReport) -> String {
    let mut text = String::new();
    text.push_str(&format!(
    ...
    text.push_str(&format!(
        "| Advisory findings | {} |\n",
        report.advisory_finding_count()
    ));
    text.push_str(&format!("| Work items | {} |\n", report.work_items.len()));
    text.push_str(&format!(
        "| Blocking-eligible work items | {} |\n",
        report.blocking_work_item_count()
    ));
    text.push_str(&format!(
        "| Advisory work items | {} |\n",
        report.advisory_work_item_count()
    ));
    text.push('\n');
    if report.findings.is_empty() {
        text.push_str(&format!(
        ...
        if report.advisory_finding_count() > 0 {
...
fn push_spec_system_report_preamble(text: &mut String, report: &SpecSystemReport) {
    ...
    text.push_str(&format!(
        "Source tree root: `{}`\n\n",
        crate::source_tree_path_text(&report.root)
    ));
...
pub fn render_spec_system_json(report: &SpecSystemReport) -> String {
    let mut text = String::new();
    text.push_str("{\n");
    text.push_str("  \"schema_version\": 1,\n");
    text.push_str(&format!(
        "  \"schema_id\": \"{}\",\n",
        crate::SPEC_SYSTEM_SCHEMA_ID
    ));
    ...
    render_string_array(&mut text, crate::SPEC_SYSTEM_CLAIM_BOUNDARY, "  ");
    text.push_str(",\n");
    text.push_str("  \"scanner_limitations\": ");
    render_string_array(&mut text, crate::SPEC_SYSTEM_SCANNER_LIMITATIONS, "  ");
    text.push_str(",\n");
    text.push_str("  \"inventory\": {\n");
    text.push_str(&format!(
        "    \"scope\": \"{}\",\n",
        crate::INVENTORY_SCOPE_SOURCE_TREE
    ));
    text.push_str(&format!(
        "    \"scanner\": \"{}\",\n",
        crate::INVENTORY_SCANNER_SOURCE_TREE_GRAPH
    ));
    text.push_str(&format!(
        "    \"source\": \"{}\",\n",
        crate::INVENTORY_SOURCE_UNKNOWN
    ));
    text.push_str(&format!(
        "    \"root\": \"{}\"\n",
        json_escape(&crate::source_tree_path_text(&report.root))
    ));
    text.push_str("  },\n");
    text.push_str(&format!(
        "  \"source_tree_root\": \"{}\",\n",
...
pub fn spec_system_proof_commands() -> Vec<String> {
    vec![
...
pub fn spec_system_mode_name(mode: &SpecSystemMode) -> &'static str {
    match mode {
...
pub fn optional_bool_json(value: Option<bool>) -> &'static str {
    match value {
        Some(true) => "true",
        Some(false) => "false",
        None => "null",
...
mod tests {
    ...
    fn render_report_dispatches_sarif_findings() {
        let rendered = render_spec_system_report(&test_report(), SpecSystemRenderFormat::Sarif);
