...
pub fn repository_snapshot_v1_from_allow_diff(
    ...
    RepositorySnapshotV1 {
        ...
        head: resolved_revision_v1_from_allow_diff(&identity.head),
        base: identity
            .base
            .as_ref()
            .map(resolved_revision_v1_from_allow_diff),
        merge_base: identity.merge_base.clone(),
        dirty_state: identity.dirty_state.as_str().to_string(),
        selected_paths: identity
            .selected_paths
            .iter()
            .map(selected_path_v1_from_allow_diff)
            .collect(),
...
fn resolved_revision_v1_from_allow_diff(identity: &ResolvedRevisionIdentity) -> ResolvedRevisionV1 {
    ResolvedRevisionV1 {
...
fn selected_path_v1_from_allow_diff(identity: &SelectedPathIdentity) -> SelectedPathIdentityV1 {
