...
- Adds `ExactCandidatePackageSetV1`: classic local-registry installation of the
  ten-crate package set with source-checkout denial, checksums, and negative
  controls before first-hour smoke.
- Adds safe atomic `add --update` and `migrate --update` active-ledger
  transactions with mutation receipts and full-policy validation.
...
Structured stale-safe `why` → `add` plans, matching parity, measured fast paths,
and the Rust 1.95 transition are planned for 0.2.0 under #2371.
