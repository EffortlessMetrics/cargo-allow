...
Published from tagged commit `d6b146b21a73613cc722297f21928bbd1ee54062`
by release workflow run `29623871988`. All ten crate versions became visible in
the crates.io index, the exact registry install smoke passed, and the GitHub
Release was created. The publish receipt records `auth = secret`; Trusted
Publishing was attempted by the workflow but was not the effective credential.
...
This patch release authenticated with the `CARGO_REGISTRY_TOKEN` fallback. The
release publish receipt records `auth = secret`. #2115 remains the required
OIDC/fail-closed transition for 0.2.0.
