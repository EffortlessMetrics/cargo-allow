...
| Valid or malformed federation registry | Federation participation and diagnostics remain separate from core fallback; malformed federation cannot become clean `NoPolicy` | Federation-aware command paths retain their existing behavior pending #3876. World loading with `require_config = false` intentionally discards the federation evaluator error and continues with an empty policy/federation result; `require_config = true` returns that evaluator error. The central resolved result retains the federation diagnostic; its status is `Partial` when a usable fallback or explicit policy remains, and otherwise reflects the direct `Invalid` or `InstrumentFailure` outcome. |
