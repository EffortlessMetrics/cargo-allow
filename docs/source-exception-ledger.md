...
`--weak-evidence`. `--status` is mutually exclusive with the status shortcuts
`--expired`/`--review-due`/`--stale`, and those shortcuts are mutually exclusive
with each other (pick one status selector). `--baseline-debt` is a
classification filter and may still be combined with one status selector.
Governed kind filters are validated up front, so a mistyped `--kind` fails
closed instead of producing a misleading empty list. Path filtering uses
normalized
