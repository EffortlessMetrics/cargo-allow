...
Here “CLI/application product identity” means the identity and capabilities by
which the cargo-allow executable presents itself. Cargo package names, versions,
publication state, selected release subjects, and other registry/release metadata
remain under CARGO-ALLOW-ADR-0003 and the checked package topology.

Cargo-allow does not own durable repository intent, the private intent graph,
proof-provider execution, cross-provider currentness, or phase-gate evidence
composition.
...
| process compatibility | first staged operation and bounded #2901 transport landed; broader semantic cutover incomplete |
...
It also owns the package/version/publication/release-subject fields that must not
be inferred from the CLI/application product identity defined in this ADR.
