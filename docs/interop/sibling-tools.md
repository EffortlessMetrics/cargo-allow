...
> Mirror maintained by cargo-allow. The canonical family document lives at [unsafe-review-swarm/docs/interop/sibling-tools.md](https://github.com/EffortlessMetrics/unsafe-review-swarm/blob/main/docs/interop/sibling-tools.md). Keep the convergence ledger and standing process synchronized when shared contracts change.
...
## cargo-allow's adoption and teaching surface

This mirror is intentionally a contract record, not a claim that cargo-allow executes sibling tools or owns the shared gate. cargo-allow remains the deterministic owned-exception-ledger instrument:

- It already accepts `unsafe-review:` evidence references as source-tree-local traceability. That links a ledger entry to sibling evidence; it does not run unsafe-review or promote that evidence into proof.
- It teaches the family the ledger-side requirements for typed evidence, classification, ownership, lifecycle, structural identity, and explicit policy dialects.
- It is adopting the shared manifest-routing vocabulary and the `new` / `worsened` / `resolved` / `inherited` movement model as the receiving side of #1471, subject to the implementation and acceptance evidence for that issue.
- It remains advisory and claim-bounded: its scan does not execute repository code, Cargo, rustc, Clippy, ripr, unsafe-review, coverage tools, or network checks.

The ledger above is the shared coordination surface. Changes to a row should land in the receiving repository's issue first, then be mirrored here and in the canonical sibling document.
