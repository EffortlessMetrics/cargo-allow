...
| workspace fmt/clippy/tests | passed | `cargo fmt --all --check`; `cargo clippy --workspace --all-targets -- -D warnings`; `cargo test --workspace` reported `1410 passed`. |
...
| ripr+ repo readiness | blocked | `ripr` explicit gap-ledger projection reported `1087` `ripr` targets and `1087` `ripr+` targets after the forty-sixth burn-down slice. |
...
`ripr+ = 1087` means the current repo does not yet meet the requested
...
1. inspect a pure parser, path, or rendering target such as
   `crates/allow-policy-legacy/src/converter_workflow_file_entries.rs` or
