...
| workspace fmt/clippy/tests | passed | `cargo fmt --all --check`; `cargo clippy --workspace --all-targets -- -D warnings`; `cargo test --workspace` reported `1446 passed`. |
...
| ripr+ repo readiness | blocked | `ripr` explicit gap-ledger projection reported `939` `ripr` targets and `939` `ripr+` targets after the sixtieth burn-down slice. |
...
`ripr+ = 939` means the current repo does not yet meet the requested
