...
| workspace fmt/clippy/tests | passed | `cargo fmt --all --check`; `cargo clippy --workspace --all-targets -- -D warnings`; `cargo test --workspace` reported `1476 passed`. |
...
| ripr+ repo readiness | blocked | `ripr` explicit gap-ledger projection reported `851` `ripr` targets and `851` `ripr+` targets after the sixty-fifth burn-down slice. |
...
`ripr+ = 851` means the current repo does not yet meet the requested
