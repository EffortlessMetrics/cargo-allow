...
| workspace fmt/clippy/tests | passed | `cargo fmt --all --check`; `cargo clippy --workspace --all-targets -- -D warnings`; `cargo test --workspace` reported `1203 passed`. |
...
| ripr+ repo readiness | blocked | `ripr` explicit gap-ledger projection reported `2755` `ripr` targets and `2755` `ripr+` targets after the second burn-down slice. |
...
`ripr+ = 2755` means the current repo does not yet meet the requested
...
1. inspect `crates/allow-report/src/report_text.rs`.
