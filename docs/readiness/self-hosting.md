...
| workspace fmt/clippy/tests | passed | `cargo fmt --all --check`; `cargo clippy --workspace --all-targets -- -D warnings`; `cargo test --workspace` reported `1553 passed`. |
...
| ripr+ repo readiness | blocked | `ripr` explicit gap-ledger projection reported `304` `ripr` targets and `304` `ripr+` targets after the eighty-sixth burn-down slice. |
...
`ripr+ = 304` means the current repo does not yet meet the requested
...
The largest remaining files are concentrated in scanner, worklist, I/O, policy,
and selector helper modules such as `line_lint_findings.rs`,
`worklist_queue.rs`, `io.rs`, `lib.rs`, and `selector.rs`. Prefer one
