...
| workspace fmt/clippy/tests | passed | `cargo fmt --all --check`; `cargo clippy --workspace --all-targets -- -D warnings`; `cargo test --workspace` reported `1361 passed`. |
...
| ripr+ repo readiness | blocked | `ripr` explicit gap-ledger projection reported `1421` `ripr` targets and `1421` `ripr+` targets after the thirty-second burn-down slice. |
...
`ripr+ = 1421` means the current repo does not yet meet the requested
...
1. inspect a pure parser, path, or rendering target such as
   ...
   `crates/allow-policy-legacy/src/parser_clippy_entries.rs`, or
