...
| workspace fmt/clippy/tests | passed | `cargo fmt --all --check`; `cargo clippy --workspace --all-targets -- -D warnings`; `cargo test --workspace` reported `1519 passed`. |
...
| ripr+ repo readiness | blocked | `ripr` explicit gap-ledger projection reported `506` `ripr` targets and `506` `ripr+` targets after the seventy-fifth burn-down slice. |
...
`ripr+ = 506` means the current repo does not yet meet the requested
...
such as `migrate_load.rs` and `reporting.rs`. Prefer one low-risk helper group
