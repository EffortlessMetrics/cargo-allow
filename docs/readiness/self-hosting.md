...
| ripr+ repo readiness | blocked | `ripr` explicit gap-ledger projection reported `34` `ripr` targets and `34` `ripr+` targets after the hundred-twenty-third burn-down slice. |
...
`ripr+ = 34` means the current repo does not yet meet the requested
...
paths such as `propose_args.rs` and `worklist_item_kind.rs`, plus allow-policy
and allow-rust scanner helpers. Prefer one low-risk helper group with direct
