...
| Legacy spec-system staged precommit | Compatibility | The selected staged-precommit compatibility operation delegates one-way to installed cargo-intent through `repo.analysis-receipt.v1`, or fails explicitly. | `scripts/spec-system-cutover-receipt.sh` and installed-candidate interop smoke | The provider subprocess is being hardened; broader semantic parity and old evaluator/schema deletion are incomplete. No silent fallback is supportable after cutover. |
...
| shared repository substrate | Experimental | The four logical shared crates exist for neutral protocol, source views, safe edits, and Rust structural indexing. | package-specific tests plus generation-2 architecture/closure validation | Cargo package identities, explicit 0.1 versions, dependency neutrality, publication posture, and direct-use support are not yet converged. |
