...
queue.
For stale cleanup work, start with the item proof commands for
...
Use `selector_precision` when present as a routing hint: lower scores mean the
selector carries fewer structural identity fields and is usually a better
target for narrowing work. Higher scores mean the selector is more specific,
but still require the normal evidence and lifecycle review. Do not treat
precision as proof that an exception is correct.
