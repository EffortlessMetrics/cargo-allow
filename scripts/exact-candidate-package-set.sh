...
if [[ "${1:-}" != "--internal" ]]; then
  ...
  if [[ -n "${CANDIDATE_HARNESS_TEST_ROOT:-}" ]]; then
    ...
    test_root_json="$(python3 "${lifecycle}" allocate --root "${TMPDIR:-/tmp}" --purpose exact-candidate-test-root)"
...
for line in os.environ["CRATE_RECORDS"].splitlines():
    ...
    crate_stem = crate_file.removesuffix(".crate")
    crate_version = crate_stem[len(name) + 1:] if crate_stem.startswith(name + "-") else crate_stem.rsplit("-", 1)[1]
    records.append(
        {
            "name": name,
