...
#
# Packages the five-crate intent stack, extracts each .crate outside the
...
#
# Usage:
#   bash scripts/intent-candidate-smoke.sh
#
# Optional:
#   WORK_DIR=<path>      work root (default: target/intent-candidate-smoke)
...
work_dir="${WORK_DIR:-${ROOT}/target/intent-candidate-smoke}"
packages_dir="${work_dir}/packages"
offline_root="${OFFLINE_ROOT:-$(mktemp -d "${TMPDIR:-/tmp}/cargo-intent-ics-offline.XXXXXX")}"
...
crate_set_fixture="${ROOT}/docs/dogfood/fixtures/release/intent-candidate-crate-set.toml"
schema_id="cargo-allow.intent-candidate-smoke.v1"
crate_set_schema_id="cargo-allow.intent-candidate-crate-set.v1"
...
log() {
  printf 'intent-candidate-smoke: %s\n' "$*"
}

fail() {
  printf 'intent-candidate-smoke: error: %s\n' "$*" >&2
  exit 1
}
...
write_patch_config() {
  ...
  local -a patched=("$@")
  {
    echo '# Generated by scripts/intent-candidate-smoke.sh'
...
extracted_bin_pkg="${extracted_dir}/cargo-intent-${intent_version}"
[[ -d "${extracted_bin_pkg}" ]] || fail "missing extracted cargo-intent package"

log "verifying extracted cargo-intent root is outside the workspace"
...
root_pkg = packages.get("cargo-intent")
if root_pkg is None:
    raise SystemExit("cargo-intent missing from metadata packages")
manifest = Path(root_pkg["manifest_path"]).resolve()
try:
    manifest.relative_to(extracted)
except ValueError as err:
    raise SystemExit(f"cargo-intent manifest not under extracted tree: {manifest}") from err
...
workspace_debug_bin="${ROOT}/target/debug/cargo-intent"
workspace_debug_bin_exe="${workspace_debug_bin}.exe"
if [[ -x "${workspace_debug_bin}" || -f "${workspace_debug_bin}" ]]; then
  log "note: workspace target/debug/cargo-intent exists; install must not use it"
fi
...
[[ "${install_code}" -eq 0 ]] || fail "cargo install from extracted cargo-intent failed (exit ${install_code})"

cargo_bin="${install_root}/bin/cargo-intent"
...
printf '%s\n' "${version_output}"
printf '%s\n' "${version_output}" | grep -F "cargo-intent ${intent_version}" >/dev/null \
  || fail "installed version mismatch: ${version_output}"

log "cargo-intent --help"
...
for pkg in meta.get("packages", []):
    ...
    if name == "cargo-intent":
        try:
            manifest.relative_to(extracted)
        except ValueError as err:
            raise SystemExit(f"cargo-intent not under extracted tree: {manifest}") from err
...
receipt = {
    ...
    "result": "Passed",
    "claim_boundary": [
        "five_crate_intent_package_graph",
...
durable_bin="${work_dir}/install/bin/cargo-intent"
...
log "IntentCandidateInstallSmokeV1 Passed for workspace ${version}"
log "receipt: ${receipt}"
