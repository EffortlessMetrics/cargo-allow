...
write_patch_config() {
  ...
  local -a patched=("$@")
  {
    ...
    for patched_crate in "${patched[@]}"; do
      path_value="$(to_cargo_path "${extracted_dir}/${patched_crate}-${version}")"
...
if [[ "${SKIP_PACKAGE:-0}" == "1" ]]; then
  ...
  for crate in "${crates[@]}"; do
    src="${packages_dir}/${crate}-${version}.crate"
...
extracted_bin_pkg="${extracted_dir}/cargo-proof-${version}"
...
printf '%s\n' "${version_output}"
printf '%s\n' "${version_output}" | grep -F "cargo-proof ${version}" >/dev/null \
