...
write_patch_config() {
  ...
  local -a patched=("$@")
  {
    ...
    for patched_crate in "${patched[@]}"; do
      path_value="$(to_cargo_path "${extracted_dir}/${patched_crate}-${crate_versions[${patched_crate}]}")"
...
if [[ "${SKIP_PACKAGE:-0}" == "1" ]]; then
  ...
  for crate in "${crates[@]}"; do
    src="${packages_dir}/${crate}-${crate_versions[${crate}]}.crate"
    ...
    crate_records+=("${crate}|${crate}-${crate_versions[${crate}]}.crate|${digest}|${size}|${crate}-${crate_versions[${crate}]}")
  done
else
  for idx in "${!dependency_crates[@]}"; do
    ...
    src="${target_dir}/package/${crate}-${crate_versions[${crate}]}.crate"
    ...
    size="$(wc -c <"${src}" | tr -d ' \r')"
    crate_records+=("${crate}|${crate}-${crate_versions[${crate}]}.crate|${digest}|${size}|${crate}-${crate_versions[${crate}]}")
    log "packaged ${crate}-${crate_versions[${crate}]}.crate sha256=${digest}"
  done
...
extracted_bin_pkg="${extracted_dir}/cargo-proof-${crate_versions[cargo-proof]}"
...
printf '%s\n' "${version_output}"
printf '%s\n' "${version_output}" | grep -F "cargo-proof ${crate_versions[cargo-proof]}" >/dev/null \
